...
queue_url = AWS.SQS.TEXTRACT_QUEUE
...
async def create_pod(message_body, logger):
    ...
    pod_manifest = json.load(open('textract/pod_manifest.json', 'r'))
...
async def runner():
    ...
    while True:
        ...
        logger = setup_cloudwatch_logging(project_id=message_body['document_path'].split('/')[2],
